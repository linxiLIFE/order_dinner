...
function PrintManagementPage({ setMessage }: { setMessage: (message: string) => void }) {
  ...
  async function load() {
    ...
    try {
      ...
      const result = await api<{ jobs: PrintJob[] }>(`/api/print-jobs?${params.toString()}`);
      setJobs(result.jobs);
    } catch (error) {
      setMessage(errorText(error));
    } finally {
  ...
  useEffect(() => { const timer = window.setTimeout(() => { void load(); }, 180); return () => window.clearTimeout(timer); }, [status, tableQuery]);
